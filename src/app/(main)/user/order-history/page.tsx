...
const Page = async () => {
  ...
  return (
    <div className="user-page-layout flex flex-col gap-2 p-2">
      {orderHistory.map((order) => (
        <div className="flex flex-col border h-fit p-2 rounded-lg" key={order.id}>
          <h3>Order ID: {order.id}</h3>
          <ul>
            {order.tickets.map((ticket) => (
              <div key={ticket.id} className="flex gap-2">
                <div className="w-[88px]">
                  <Image src={ticket.movie.poster_url} alt={ticket.movie.title} width={100} height={100}/>
                </div>
                <li>
